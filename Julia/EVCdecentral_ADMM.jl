...
for p in 1:numIteration-1
	...
	#for evInd=1:N
		...
		@objective(evM,Min, sum(objFun(xn,u)+sum(lambda[k,1]*(u[k,1]-evV[k,1]) for k=1:horzLen+1)+rhoADMM/2*sum((u[k,1]-evV[k,1])^2 for k=1:horzLen+1)))
    ...
    constFun2(u,v)=rhoADMM/2*sum(sum((u[(k-1)*(S)+s,1]-v[(k-1)*(S)+s,1])*(u[(k-1)*(S)+s,1]-v[(k-1)*(S)+s,1]) for s=1:S)  for k=1:(horzLen+1))
	...
	for k=1:horzLen+1
		...
		Lam[k,p+1]=Lam[k,p]+rhoADMM/(S*(N))*(currConst[k,1])
	end
