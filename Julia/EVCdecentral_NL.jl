...
for p=1:maxIt-1
	...
	fGap= objFun(dLog.Sn[:,p+1],dLog.Un[:,p+1])-fStarNL
	snGap=norm((dLog.Sn[:,p+1]-snStarNL),2)
	unGap=norm((dLog.Un[:,p+1]-uStarNL),2)
	itGap = norm(dLog.Lam[:,p+1]-dLog.Lam[:,p],2)
	if updateMethod=="fastAscent"
		convGap = norm(dLog.Lam[:,p+1]-lamTempStarNL,2)
	else
		convGap = norm(dLog.Lam[:,p+1]-lamCurrStarNL,2)
	end
...
pd3nl=plot(layer(yintercept=[evS.Tmax],Geom.hline(color=["red"],style=:dot),Theme(line_width=3pt)),
		...
		minor_label_font_size=20pt,key_label_font_size=20pt))
if updateMethod=="dualAscent"
	push!(pd3nl,layer(x=1:horzLen+1,y=dLog.Xt[:,convIt],Geom.line,Theme(default_color=colorant"blue",line_width=3pt)))
...
pd4nl=plot(layer(x=1:horzLen+1,y=dLog.Lam[:,convIt],Geom.line,Theme(line_width=3pt)),
...
uSumPlotNL=plot(dLog.uSum[:,2:convIt],x=Row.index,y=Col.value,color=Col.index,Geom.line,
...
iPlotNL=plot(dLog.Itotal[:,2:convIt],x=Row.index,y=Col.value,color=Col.index,Geom.line,
			...
			Coord.Cartesian(xmin=0,xmax=horzLen+1),Theme(background_color=colorant"white",major_label_font_size=30pt,line_width=2pt,
			minor_label_font_size=26pt,key_label_font_size=26pt))
lamPlotNL=plot(dLog.Lam[:,1:convIt],x=Row.index,y=Col.value,color=Col.index,Geom.line,
...
fPlotNL=plot(x=1:convIt-1,y=dCM.objVal[1:convIt-1,1],Geom.line,Scale.y_log10,
			...
			minor_label_font_size=26pt,key_label_font_size=26pt))
convItPlotNL=plot(x=1:convIt,y=dCM.lamIt[1:convIt,1],Geom.line,Scale.y_log10,
			Guide.xlabel("Iteration"), Guide.ylabel("2-Norm Lambda Gap",orientation=:vertical),
			Coord.Cartesian(xmin=0,xmax=convIt),Theme(background_color=colorant"white",major_label_font_size=30pt,line_width=2pt,
			minor_label_font_size=26pt,key_label_font_size=26pt))
convPlotNL=plot(x=1:convIt,y=dCM.lam[1:convIt,1],Geom.line,Scale.y_log10,
			Guide.xlabel("Iteration"), Guide.ylabel("Lambda Star Gap",orientation=:vertical),
			Coord.Cartesian(xmin=0,xmax=convIt),Theme(background_color=colorant"white",major_label_font_size=30pt,line_width=2pt,
			minor_label_font_size=26pt,key_label_font_size=26pt))
constPlotNL=plot(x=1:convIt,y=dCM.couplConst[1:convIt,1],Geom.line,Scale.y_log10,
