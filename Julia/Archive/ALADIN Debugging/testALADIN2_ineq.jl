...
for p=1:maxIt-1
    ...
    for i=1:N
        ...
        @variable(nlp,y)
        @objective(nlp,Min,0.5*y^2*Q[i,1]+Lambda[p,1]*A[i,1]*y+rhoALAD/2*(y-X[i,p])^2*H[i,1])
    ...
    @variable(mC,dy[1:N])
    @objective(mC,Min,sum(0.5*dy[i,1]^2*H[i,1]+G[i,p+1]*dy[i,1] for i=1:N))
    @constraint(mC,lambdaP,sum(A[i,1]*(Y[i,p+1]+dy[i,1]) for i=1:N)==b)
    @constraint(mC,C.*dy.==0)
    TT = STDOUT # save original STDOUT stream
    redirect_stdout()
    status = solve(mC)
    redirect_stdout(TT)
