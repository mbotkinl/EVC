...
println("constraints")
@constraint(m,stateCon1[n=1:N],sn[n,1]==sn0[n,1]+etaP[n,1]*u[n,1])
@constraint(m,stateCon2[k=1:horzLen,n=1:N],sn[k*N+n,1]==sn[(k-1)*N+n,1]+etaP[n,1]*u[k*N+n,1])
@constraint(m,tempCon1,xt[1,1]==tauP*xt0+gammaP*(itotal[1])^2+rhoP*w[stepI*2,1])
@constraint(m,tempCon2[k=1:horzLen],xt[k+1,1]==tauP*xt[k,1]+gammaP*(itotal[k+1])^2+rhoP*w[stepI*2+k*2,1])
