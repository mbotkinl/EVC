...
function runHubDualIt(p,stepI,hubS,dLog,dSol,cSol,mode,silent)
    ...
    dLog.Lam[:,1,p]=round.(prevLam[:,1]+alphaP*dLog.couplConst[:,1,p],digits=4)
...
function localEVALAD(hubInd::Int,p::Int,stepI::Int,σU::Array{Float64,2},σE::Array{Float64,2},σD::Array{Float64,2},hubS::scenarioHubStruct,dLogalad::hubItLogPWL)
    ...
    dLogalad.E[:,hubInd,p]=round.(eVal,digits=6)
    dLogalad.U[:,hubInd,p]=round.(uVal,digits=6)

    dLogalad.Gu[:,hubInd,p]=round.(2*hubS.Rh[hubInd]*uVal,digits=4)
    dLogalad.Ge[:,hubInd,p]=round.(2*hubS.Qh[hubInd]*eVal.-2*hubS.Qh[hubInd]*eMax,digits=4)
    #dLogalad.GeΔ[:,hubInd,p].=0
    return nothing
end

function localXFRMALAD(p::Int,stepI::Int,σZ::Float64,σT::Float64,hubS::scenarioHubStruct,dLogalad::hubItLogPWL,mode::String)
    S=hubS.S
    horzLen=min(hubS.K1,hubS.K-stepI)
    ...
    dLogalad.Z[:,:,p]=round.(zVal,digits=6)
    dLogalad.Gz[:,:,p].=0
    dLogalad.Gt[:,:,p].=0
...
function coordALAD(p::Int,stepI::Int,μALADp::Float64,hubS::scenarioHubStruct,dLogalad::hubItLogPWL)
    ...
    dLogalad.Lam[:,1,p]=round.(prevLam[:,1]+α3*(-getdual(currCon)-prevLam[:,1]),digits=6)
    #dLogalad.Lam[:,p]=max.(prevLam[:,1]+α3*(-getdual(currCon)-prevLam[:,1]),0)
    dLogalad.Vu[:,:,p]=round.(prevVu[:,:,1]+α1*(dLogalad.U[:,:,p]-prevVu[:,:,1])+α2*getvalue(dU),digits=6)
    dLogalad.Vz[:,:,p]=round.(prevVz[:,:,1]+α1*(dLogalad.Z[:,:,p]-prevVz[:,:,1])+α2*getvalue(dZ),digits=6)
    dLogalad.Ve[:,:,p]=round.(prevVe[:,:,1]+α1*(dLogalad.E[:,:,p]-prevVe[:,:,1])+α2*getvalue(dE),digits=6)
    dLogalad.Vd[:,:,p]=round.(prevVd[:,:,1]+α1*(dLogalad.Vd[:,:,p]-prevVd[:,:,1])+α2*getvalue(dEΔ),digits=6)
    dLogalad.Vt[:,:,p]=round.(prevVt[:,:,1]+α1*(dLogalad.Tpwl[:,:,p]-prevVt[:,:,1])+α2*getvalue(dT),digits=6)
...
function runEVALADIt(p,stepI,hubS,dLogalad,dSol,cSol,mode,eqForm,silent)
    H=hubS.H
    K=hubS.K
    S=hubS.S
    horzLen=min(hubS.K1,K-stepI)
    ...
    cc=norm(hcat(σU[1]*(prevVu[:,:]-dLogalad.U[:,:,p]),σZ*(prevVz[:,:]-dLogalad.Z[:,:,p]),
                 σT*(prevVt[:,:]-dLogalad.Tpwl[:,:,p]),σE[1]*(prevVe[:,:]-dLogalad.E[:,:,p])),1)
    ...
    coordALAD(p,stepI,μALADp,hubS,dLogalad)
...
function runHubALADStep(stepI,maxIt,hubS,dSol,cSol,mode,eqForm,silent)
    ...
    for p=1:maxIt
        #@printf "%git" p
        cFlag=runEVALADIt(p,stepI,hubS,dLogalad,dSol,cSol,mode,eqForm,silent)
        global convIt=p
        if cFlag
            break
        end
