...
plot!(p3pem,hubS.Tmax*ones(hubS.K),label="XFRM Limit",line=(:dash,:red),linewidth=2,xticks=xticks)
if drawFig savefig(p3pem,path*"J_PEM_Temp.png") end

requests = [count(Req[k,:].>0) for k=1:hubS.K]
reqPlot=plot(requests,xlims=(0,hubS.K),xlabel="Time",ylabel="Requests",legend=false,linewidth=2,linecolor=:purple,xticks=xticks)
...
aggU=plot(hcat(cSol.uSum .+ hubS.iD_actual,pemSol.Itotal,hubS.iD_actual),label=["Central" "Total" "iD"],xlims=(0,hubS.K),
			xlabel="Time",ylabel="Current (kA)",linewidth=2,xticks=xticks)
